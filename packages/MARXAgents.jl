...
export MARXAgent, update!, predictions, posterior_predictive, EFE, crossentropy, mutualinfo, minimizeEFE, update_goals!, reset_buffer!
...
function update!(agent::MARXAgent, y_k::Vector, u_k::Vector)
    ...
    agent.free_energy = -log_evidence(agent, y_k, x_k)
end
...
function log_evidence(agent::MARXAgent, y,x)
...
function save_agent(
    ...
    if ~isdir(directory) && directory != ""
        ...
        if makedir
            ...
            error("Directory $directory does not exist. Agent cannot be saved to $filename")
        end	
    end
    JLD.save(filename, "agent", agent)
    @info "Agent saved to $filename"
end


function load_agent(file::String)
...
function reset_buffer!(agent::MARXAgent)

    agent.ubuffer = zeros(agent.Du, agent.delay_inp)
    agent.ybuffer = zeros(agent.Dy, agent.delay_out)
