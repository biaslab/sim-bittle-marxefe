...
def other_stuff():
  if self._enable_debug_prints: print('CONDITION 1')
  ...
  if self._accurate_motor_model_enabled:
    ...
    if self._motor_overheat_protection:
      for i in range(self.num_motors):
        self._overheat_counter[i] = 0 if abs(actual_torque[i]) <= self._overheat_shutdown_torque else self._overheat_counter[i] + 1
        if (self._overheat_counter[i] > self._overheat_shutdown_time / self.time_step):
          self._motor_enabled_list[i] = False

    # The torque is already in the observation space because we use GetTrueMotorAngles and GetTrueMotorVelocities.
    ...
    for motor_id, motor_torque, motor_enabled in zip(self._motor_id_list, self._applied_motor_torque, self._motor_enabled_list):
      self._SetMotorTorqueById(motor_id, motor_torque if motor_enabled else 0)
  else:
